...
const updateAppointment = catchError(async (req, res, next) => {
    ...
    const dataToUpdate = {};
    if (patientId)
        dataToUpdate.patientId = patientId;
    if (doctorId)
    ...
    if (!req.params.id)
        return next(new appError('Appointment ID is required', 401));
...
const getSingleAppointment = catchError(async (req, res, next) => {
    if (!req.params.id)
        return next(new appError('Appointment ID is required', 401));
...
const getAllAppointments = catchError(async (req, res, next) => {
    const qb = new QueryBuilder(getPrisma.appointment, req.query, ['notes']);
    await qb
        .filter()
        .search()
        .sort()
