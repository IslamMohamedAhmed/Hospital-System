...
const register = catchError(async (req, res, next) => {
    ...
    if (email)
        return next(new appError('email has to be unique & that email was used before!!', 401));
...
const verify = catchError(async (req, res, next) => {
    ...
    jwt.verify(token, env('JWT_KEY_SIGNUP'), async (err, decoded) => {
        if (err)
...
const changePassword = catchError(async (req, res, next) => {
    let userData = req.headers['user-info'];
...
const resetPassword = catchError(async (req, res, next) => {
    ...
    let token = Array.isArray(tokenHeader) ? tokenHeader[0] : tokenHeader;
    if (!token)
        return next(new appError('Token is required', 400));
    let decoded = jwt.verify(token, env('JWT_KEY_RESET_PASSWORD'));
    if (!decoded.userId)
...
const updateUser = catchError(async (req, res, next) => {
    ...
    if (role)
        dataToUpdate.role = role; // make sure role is validated if enum
