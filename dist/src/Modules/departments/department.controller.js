...
const addDepartment = catchError(async (req, res, next) => {
    ...
    if (name)
        return next(new appError('Department name has to be unique & that name was used before!!', 401));
    await getPrisma.department.create({
        data: req.body
    });
    res.status(201).json({ message: "department was added successfully!!" });
});
const updateDepartment = catchError(async (req, res, next) => {
    const { name } = req.body;
    const dataToUpdate = {};
    if (name) {
    ...
    if (!req.params.id)
        return next(new appError('Department ID is required', 401));
...
const getSingleDepartment = catchError(async (req, res, next) => {
    if (!req.params.id)
        return next(new appError('Department ID is required', 401));
...
const deleteDepartment = catchError(async (req, res, next) => {
    if (!req.params.id)
        return next(new appError('Department ID is required', 401));
...
const getAllDepartments = catchError(async (req, res, next) => {
    const qb = new QueryBuilder(getPrisma.department, req.query, ['name']);
    await qb
        .filter()
        .search()
        .sort()
