...
PatientRouter.route('/:id')
    .get(validateToken, validateInputs(validateParamsId), findById({ model: getPrisma.patient, foreignKey: 'id', from: 'params', necessary: true, objectName: 'patient' }), getSinglePatient)
    .delete(validateToken, givePermissionTo(Role.ADMIN), validateInputs(validateParamsId), findById({ model: getPrisma.patient, foreignKey: 'id', from: 'params', necessary: true, objectName: 'patient' }), deletePatient)
    .put(validateToken, givePermissionTo(Role.ADMIN), validateInputs(updatePatientValidation), findById({ model: getPrisma.patient, foreignKey: 'id', from: 'params', necessary: true, objectName: 'patient' }), updatePatient);
